...
<script>let d;const scene=document.querySelector('#scene'),video=document.querySelector('#video'),stage=document.querySelector('#stage');fetch('/api/state').then(r=>r.json()).then(x=>{d=x;document.querySelector('#meta').innerHTML=`<b>${x.model_type}</b> · ${x.parameter_count.toLocaleString()} parameters<br>Command: ${x.command}<br>Champion: ${x.champion_result.success}/${x.champion_result.n} · round ${x.champion_round}<br>Checkpoint SHA: ${x.checkpoint_sha256||'not persisted'}<br>Artifact SHA: ${x.artifact_sha256}<br><span class=muted>${x.checkpoint_status}</span>`;x.scenes.forEach(s=>{let o=document.createElement('option');o.value=s.scene;o.textContent='Scene '+s.scene;scene.append(o)});show()});function show(){let s=d.scenes[scene.value||0];video.src=s.video||'';let m=s.metrics;stage.innerHTML=`Scene ${s.scene}: <span class=${m.success?'ok':'bad'}>${m.success?'success':'failure'}</span> · approach ${m.approach?'✓':'✗'} · grasp ${m.grasp?'✓':'✗'} · transport ${m.transport?'✓':'✗'} · release ${m.release?'✓':'✗'}`};scene.onchange=show;document.querySelector('#replay').onclick=()=>{video.currentTime=0;video.play()};</script>"""
