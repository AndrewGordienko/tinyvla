...
def h(*xs):
 ...
 return d.hexdigest()
def snap(env):
 return {'qpos':env.data.qpos.copy(),'qvel':env.data.qvel.copy(),'act':env.data.act.copy(),'ctrl':env.data.ctrl.copy(),'time':float(env.data.time),'grasped':env.grasped,'phase':env.phase,'phase_t':env.phase_t,'step_idx':env.step_idx,'steps':copy.deepcopy(env.steps),'instruction':env.instruction,'rng':copy.deepcopy(env.rng.bit_generator.state),'off_pos':getattr(env,'_off_pos',None),'off_quat':getattr(env,'_off_quat',None)}
def restore(env,s):
 env.data.qpos[:]=s['qpos']; env.data.qvel[:]=s['qvel']; env.data.act[:]=s['act']; env.data.ctrl[:]=s['ctrl']; env.data.time=s['time']; env.grasped=s['grasped']; env.phase=s['phase']; env.phase_t=s['phase_t']; env.step_idx=s['step_idx']; env.steps=copy.deepcopy(s['steps']); env.instruction=s['instruction']; env.rng.bit_generator.state=copy.deepcopy(s['rng']);
